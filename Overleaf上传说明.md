...
打开 Overleaf 并登录，在项目首页选择 New project，再选择 Existing project (.zip)。在 Upload Zipped Project 窗口中拖入或选择项目根目录中的 `完整论文-Overleaf.zip`。上传成功后项目会自动打开。压缩包解压后，`example.tex`、`cumcmthesis.cls`、`cumcm2026.sty` 和 `figures` 文件夹应位于项目顶层。
