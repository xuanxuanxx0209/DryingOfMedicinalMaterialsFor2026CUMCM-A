...
\begin{abstract}
圆柱形中药材热风烘干同时受时变空气边界、温湿相关物性和尺寸收缩影响，表面状态难以代表内部温度--含水率场。围绕“预热场重构--全程变物性演化--固定域终点--收缩域终点”四层任务，本文以 Fourier 导热和 Fick 扩散为基础，建立一维轴对称热质传递模型，并以全域最大干基含水率严格低于 \(0.15\,\mathrm{kg/kg}\) 作为统一干燥终点判据。
...
\textbf{针对问题三，}以 \(9000\!\sim\!14400\,\mathrm{s}\) 稳定段的 91 个观测值均值延拓空气边界，并逐秒检查全部径向节点。得到\textbf{固定半径烘干时长为 \(57.4322\,\mathrm{h}\)，终点由轴线控制}，终点中心含水率的未舍入值为 \(0.14999973\,\mathrm{kg/kg}\)。在稳定段均值上下各扰动一个总体标准差及末条记录替代等五个长期空气边界情景中，时长最大绝对相对变化为 \(0.5117\%\)；空间网格加密后的时长相对差为 \(0.09044\%\)。

\textbf{针对问题四，}将实测半径作为移动边界，建立\textbf{移动材料坐标模型}，在固定单位区间内更新几何权重和附录四物性。收缩模型在 \(51.0775\,\mathrm{h}\) 达到终点，此时半径为 \(1.2000\,\mathrm{cm}\)；同附录四物性的固定半径对照需 \(129.7964\,\mathrm{h}\)，故\textbf{观测半径收缩使模型终点缩短 \(78.7189\,\mathrm{h}\)，缩短比例为 \(60.6480\%\)}。所检验非均匀映射族的最大结构偏差为 \(0.3143\%\)，空间网格与时间步加密的终点绝对差分别为 \(13\,\mathrm{s}\) 和 \(0.5\,\mathrm{s}\)。离散平衡、时空步加密及边界与结构敏感性结果共同表明，关键结论在本文设定的数值与情景范围内稳定。
...
热风接触药材表面后，热量由外层向轴线传入，水分沿相反方向迁移。两类梯度共同控制升温和失水过程。连续介质模型以温度和含水率为场变量，可以描述药材内部的时空分布\cite{liu2020}。

题面给出的空气状态随时间变化，只依据表面状态不能确定内部响应。圆柱物料的对流干燥可用非稳态热传导方程和水分扩散方程描述\cite{tzempelikos2015}。Hussain 和 Dincer 的轴对称模型保留径向及轴向梯度，为判断本题能否降为径向问题提供了参照\cite{hussain2003}。

药材长度为 \SI{25}{cm}，初始半径为 \SI{2}{cm}，题目所列观测位置均以到轴线的距离表示。固定尺寸阶段据此采用一维径向模型。已有圆柱形产品研究使用有限体积法求解热质传递方程，并以第三类边界描述表面对流\cite{silva2014}。问题四再把实测半径写入移动边界。
...
附件一在 \SI{14400}{s} 结束，而问题三的计算范围超过该时刻。取 \SI{9000}{s} 至 \SI{14400}{s} 的九十一个观测点计算稳定段均值，用它延拓后续空气边界。该处理保留平台水平，并降低单条记录对延拓值的影响。
...
药材在各截面上视为同轴圆形，初始长度和半径分别取 \SI{0.25}{m} 与 \SI{0.02}{m}。材料内部不设置孔洞、裂纹和局部空腔，温度及含水率只随径向坐标和时间变化。端面面积占换热总表面积的比例为 \(R/(L+R)\)，其数值约为 \(0.074\)，模型忽略端部通量。
...
温度结果应理解为无显式潜热源项条件下的计算值。潜热对相近食品干燥模型拟合精度的影响由文献中的对照数据说明，具体数值列在模型评价中。

\subsection{表面交换}

烘房空气在药材表面附近视为空间均匀，附件一的温度和水分浓度可代表整段侧壁的环境状态。表面对流换热系数和对流传质系数在问题一中取附录二数值，表面不存在额外热阻和质阻。

题面把药材干基含水率和空气水分浓度均记为 \si{kg/kg}。模型按题设口径直接用二者差值构造传质驱动力，不再转换平衡含水率。两者基准物质未在题面中分别定义，因此该边界只按题设经验关系解释，其适用范围在模型评价中说明。
...
问题一的基准内部网格取 \(\Delta r=\SI{0.000125}{m}\)，时间步取 \(\Delta t=\SI{0.25}{s}\)。程序每隔 \SI{1}{s} 保存一次，并把内部节点插值到题目要求的 \SI{0.1}{cm} 空间间隔。问题二沿用 \SI{1}{s} 输出间隔。问题三采用三百二十个径向区间和 \SI{1}{s} 时间步，每隔 \SI{60}{s} 保存一次，并保留终点短步。空间网格及时间步加密结果用于检验离散精度。
...
\end{equation}
相邻两次迭代的含水率最大差不超过 \(10^{-10}\) 后进入下一时间层，单步最多迭代五十次。计算过程中实际最多进行四次迭代，全部时间层均达到收敛容差。线性方程组采用 Thomas 算法求解，基准网格下热量矩阵与水分矩阵的条件数分别为 \(1.889\times10^3\) 和 \(1.254\times10^3\)，均低于病态判据 \(10^7\)。

基准计算取 \(\Delta r=\SI{0.000125}{m}\) 与 \(\Delta t=\SI{0.25}{s}\)，对应一百六十个径向小区间和七千二百个时间步。程序每隔四个内部时间步输出一次，并将内部网格线性抽取到题目要求的 \SI{0.1}{cm} 间隔。温度以摄氏度输出，全部结果按题目模板保留四位小数。

% 待插图：径向有限体积网格及中心、表面控制体。

\subsubsection{问题一的模型计算结果}

表 \ref{tab:q1-temperature} 与表 \ref{tab:q1-moisture} 给出题目指定位置的计算值。药材表面温度在 \SI{1800}{s} 时达到 \SI{36.7857}{\degreeCelsius}，中心温度为 \SI{33.5756}{\degreeCelsius}，径向温差为 \SI{3.2101}{\degreeCelsius}。同一时刻表面含水率降至 \SI{1.5103}{kg/kg}，中心含水率按四位小数仍为 \SI{2.5500}{kg/kg}，两者相差 \SI{1.0397}{kg/kg}。
...
基准计算把半径等分为一百六十个区间，\(\Delta r=\SI{0.000125}{m}\)，时间步取 \(\Delta t=\SI{1}{s}\)。程序保存一秒至一万零八百秒的每个时间层，并把内部解线性插值到零至二厘米的二十一个输出位置，空间间隔为 \SI{0.1}{cm}。温度表和水分浓度表各含一万零八百行数据及二十二列，表中数值保留四位小数。
...
全域最大含水率在 \SI{206756}{s} 首次低于阈值，对应固定半径烘干时长 \SI{57.4322}{h}。前一常规记录 \SI{206700}{s} 的最大含水率为 \SI{0.15001607}{kg/kg}，终点值为 \SI{0.14999973}{kg/kg}。图 \ref{fig:q3-threshold} 给出全程轨迹，并将前一常规记录及终点的 \(C_{\max}-0.15\) 单独绘出。终点偏差为 \(-2.669\times10^{-7}\,\si{kg/kg}\)，严格低于零；首次满足条件的时刻由逐秒检查确定，阈值跨越由中心含水率控制。
...
基准解的最大含水率反向增量为 \SI{3.9968e-15}{kg/kg}，水分平衡最大相对残差为 \(6.8716\times10^{-10}\)。温度与含水率线性系统的最大相对残差分别为 \(3.0249\times10^{-14}\) 和 \(2.4208\times10^{-15}\)，末时刻条件数分别为 \(8386.07\) 和 \(1771.93\)。全部时间层最多用四次 Picard 迭代。这些误差低于相应输出精度，且未改变离散时间步上的终点判定。

稳定温度减少和增加一个总体标准差时，烘干时长分别变化 \SI{0.4904}{\percent} 和 \SI{-0.4875}{\percent}；空气水分浓度作相同方式扰动时，变化幅度不超过 \SI{0.0063}{\percent}。采用附件末条记录会使时长缩短 \SI{0.5117}{\percent}。按端面与侧面面积比把换热及传质系数同时放大 \SI{8}{\percent} 后，时长缩短 \SI{0.8372}{\percent}。图 \ref{fig:q3-sensitivity} 表明长期边界选择和端面简化对本问结论的影响均在百分之一以内。
...
\end{equation}
结果文件每隔 \SI{60}{s} 保存一次，并额外保存终点。固定实际位置通过 \(x=r/R(t)\) 映射到当前网格，超出当前半径的位置记为域外，另设一列记录实时表面值。
...
内部映射参数取 0.1 和 0.2 时，干燥时长分别为 \SI{51.1572}{h} 和 \SI{51.2381}{h}。相对主模型的变化为 \SI{0.1561}{\percent} 和 \SI{0.3143}{\percent}，最大结构偏差为 \SI{0.3143}{\percent}。图 \ref{fig:q4-shape} 左幅给出主模型终点时刻的假设映射偏离 \(r/R-\xi\)，两端均为零，表示各映射共享中心和实测外半径；这不是内部位移的观测结果。右幅两组非均匀映射的干燥时长分别比主模型增加 \SI{287}{s} 和 \SI{578}{s}。
...
\caption{内部映射扰动及其对干燥时长的确定性影响；左图取 \(t=51.0775\,\mathrm{h}\)，右图为未标定情景的计算结果}
\label{fig:q4-shape}
\end{figure}
\FloatBarrier
...
指标 \(E_{\mathrm{air}}\) 检验长期空气边界的取值，指标 \(E_{\mathrm{shape}}\) 检验同一外半径下的内部收缩映射。两者采用相同主模型时长作比较基准，但情景集合不同。
...
问题三的长时空气边界采用稳定段均值。总体标准差扰动和末条记录替代使终点时长变化不超过 \SI{0.5117}{\percent}。问题四按式 \ref{eq:q4-air-error} 检验五个空气边界情景，最大绝对时长变化为 \(E_{\mathrm{air}}=0.5058\%\)，对应附件末条记录替代情景。主模型在 \SI{51.0775}{h} 结束，早于半径观测终点，因而没有调用末值保持规则。问题四只观测外表面半径，内部映射参数没有实验标定。两组结构扰动给出 \(E_{\mathrm{shape}}=0.3143\%\)，该数值只适用于式 \ref{eq:q4-nonuniform-map} 的映射族。缺少内部位移和热质通量观测时，现有数据不能辨识局部孔隙演化或多相传递参数，因此该指标不是所有收缩机制的不确定性上界。
