...
\textbf{问题一：}以长 \SI{25}{cm}、初始半径 \SI{2}{cm} 的圆柱形药材为研究对象，利用附件一的烘房温度、空气水分浓度和附录二的参数，建立预热平衡阶段的温度与干基含水率变化模型。药材初始温度为 \SI{28}{\degreeCelsius}，初始干基含水率为 \SI{2.55}{kg/kg}。除计算指定时刻和指定径向位置的结果外，还需按 \SI{1}{s} 的时间间隔、\SI{0.1}{cm} 的空间间隔输出前 \SI{1800}{s} 的完整结果。

\textbf{问题二：}在问题一的基础上，区分预热平衡与恒温干燥阶段的参数，并采用附录三给出的物性经验式，建立约 2--3 天全程烘干过程中温度与干基含水率的变化模型。给出前 \SI{3}{h} 内每隔 \SI{0.5}{h}、距轴线每隔 \SI{0.5}{cm} 的计算结果，并按 \SI{1}{s} 的时间间隔、\SI{0.1}{cm} 的空间间隔输出完整结果。
...
\textbf{问题四：}进一步考虑失水引起的尺寸变化，将附件二的半径记录和附录四的物性经验式纳入模型，沿用问题三的结束判据重新确定烘干时长。给出每隔 \SI{6}{h}、距轴线每隔 \SI{0.5}{cm} 以及药材表面的含水率，并按 \SI{60}{s} 的时间间隔、\SI{0.1}{cm} 的空间间隔输出完整结果。
...
药材长径比为 \(L/R=12.5\)，两个端面总面积与侧面积之比为 \(R/L=0.08\)。该比例只反映端面与侧面的几何量级，不能直接作为轴向简化的误差上界。结合题目只要求径向位置结果这一信息，本文采用轴对称一维径向模型，并把忽略轴向梯度列为结构性近似。温度场满足 Fourier 导热定律，水分场满足 Fick 扩散定律，表面分别施加对流换热和对流传质边界。

问题一的物性参数均由附录二给定。温度方程为线性扩散方程，水分扩散系数随含水率变化，使水分方程呈非线性。径向有限体积法使相邻控制体的共享通量在离散层面相互抵消，并利用中心控制体的零面积内边界消除圆柱坐标在轴线处的形式奇点。这里的“守恒”是所建有效方程的离散平衡，不等同于已经验证真实干固体和水分的质量库存。
...
干燥结束条件约束的是药材所有位置，而非某个抽样位置。对每个判定时间层计算径向最大含水率，可以避免未经验证就把中心点当作控制点。若数值结果表明含水率沿半径单调下降，最大值才可由中心含水率代替。

附件一在 \SI{14400}{s} 结束，而问题三的计算范围超过该时刻。取 \SI{9000}{s} 至 \SI{14400}{s} 的九十一个观测点计算稳定段均值；在烘房此后继续维持同一平台工况的条件下，用该均值延拓后续空气边界。该处理保留平台水平，并降低单条记录对延拓值的影响，但仍属于超出实测时段的工况假设。
...
半径数据在 \SI{259200}{s} 结束，末段已保持 \SI{1.198}{cm}。若计算终点晚于该时刻，模型令半径维持末值，不进行无依据外推。固定尺寸对照与移动域主模型均采用附录四物性，并保持环境、初值及数值算法一致。内部位移未被观测，相似收缩闭合还需在同一外半径下用非均匀映射检验。

\section{模型假设}

\subsection{几何简化}

药材在各截面上视为同轴圆形，初始长度和半径分别取 \SI{0.25}{m} 与 \SI{0.02}{m}。材料内部不设置孔洞、裂纹和局部空腔，温度及含水率只随径向坐标和时间变化。端面面积占换热总表面积的比例为 \(R/(L+R)\)，其数值约为 \(0.074\)。模型据此忽略端部通量，但该几何比例只支持降维假设，不能证明局部轴向偏差具有同一上界。
...
任一时刻的同一径向控制体内视为均匀连续介质，局部温度和干基含水率可以用单值函数表示。问题一采用附录二中的常密度、常比热容和常导热系数，问题二至问题四按题面经验式逐点更新物性。问题一固定域内还把干固体参考密度视为时空常数；以干基含水率为变量时，该常数在储存项和扩散通量两侧相消，因此水分方程不另列密度因子。

模型不计内部化学反应热、辐射换热和药材与托盘之间的接触传热。水分迁移采用等效液态扩散描述，题面给出的扩散系数作为孔隙尺度迁移的综合参数。该参数只进入水分方程，不代替温度方程中的汽化能耗。

\subsection{潜热处理}

题面附录给出了密度、比热容、导热系数、换热系数、传质系数和水分扩散系数，没有给出水的汽化潜热，也没有给出局部相变速率或相间传质关系。缺少这两类信息时，温度方程中的相变源项不能由题面参数唯一确定。

本文采用的 Hussain 和 Dincer 参考方程把热传导与水分扩散分别建模，未显式列出汽化潜热源项\cite{hussain2003}。为避免引入题面无法确定的参数并沿用参考方程形式，基准模型不显式加入潜热。这个选择只规定了模型口径，不表示汽化能耗可以忽略。

温度结果应理解为无显式潜热源项条件下的计算值。问题一的热物性为常数，且扩散系数只依赖含水率，因此温度场与含水率场在该问中按两条相互独立的有效方程分别求解，不把它表述为包含相变能量交换的严格热湿耦合。潜热对相近食品干燥模型拟合精度的影响由文献中的对照数据说明，具体数值列在模型评价中。

\subsection{表面交换}

烘房空气在药材表面附近视为空间均匀，附件一的温度和水分浓度可代表整段侧壁的环境状态。表面对流换热系数和对流传质系数在问题一中取附录二数值，表面不存在额外热阻和质阻。

题面把药材干基含水率和空气水分浓度均记为 \si{kg/kg}，但没有分别说明两者的基准物质。相同单位并不足以证明二者是可直接比较的同一物理浓度。本文把给定的 \(h_m\) 与差值 \(C-C_a\) 组合解释为题设口径下的等效经验传质关系，不将其外推为一般的湿空气--药材平衡关系；若获得平衡含水率或吸附等温线，应替换表面驱动力并重新求解。

问题三在附件一结束后采用稳定段均值作为空气边界。温度取 \SI{50.004945}{\degreeCelsius}，空气水分浓度取 \SI{0.049993736}{kg/kg}。稳定段总体标准差及末条记录替代工况用于检验这一延拓对终点时长的影响；这些扰动是确定性情景，不解释为统计置信区间。

\subsection{数值离散}

径向控制体边界与轴线同心，单元内物性由节点值代表。控制面导热系数取相邻节点值的算术平均，水分扩散系数采用同样规则。问题一采用全隐式时间格式，问题二从第二个时间层起采用二阶向后差分，非线性物性在单个时间层内迭代更新。

问题一的基准内部网格取 \(\Delta r=\SI{0.000125}{m}\)，时间步取 \(\Delta t=\SI{0.25}{s}\)。程序每隔 \SI{1}{s} 保存一次，并把内部节点插值到题目要求的 \SI{0.1}{cm} 空间间隔。问题二沿用 \SI{1}{s} 输出间隔。问题三采用三百二十个径向区间和 \SI{1}{s} 时间步，每隔 \SI{60}{s} 保存一次，并单独追加终点记录。空间网格及时间步加密结果用于检验离散精度。
...
\end{equation}
温度方程取 \(u=T\)、\(\beta=\rho c_p\)、\(\Gamma=k\)，水分方程取 \(u=C\)、\(\beta=1\)、\(\Gamma=D\)。相邻节点的物性算术平均值代表控制面物性，同一内部控制面的通量在相邻控制体中符号相反，求和后能够抵消。这一性质保证所建有效场方程在离散层面的通量平衡；由于模型未另行定义干固体质量库存和含湿总焓，它不表示已经验证真实水分质量或总焓守恒。
